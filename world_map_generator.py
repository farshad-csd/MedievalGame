...
class WorldMapGenerator:
    ...
    def paint_territory_tint(self):
        territory_colors = {0: C['territory_red'], 1: C['territory_blue'], 2: C['territory_green']}
        cells_per = BIOME_GRID // CELL_SIZE
        for kid, territory in self.kingdom_territories.items():
            tint = territory_colors.get(kid, C['territory_red'])
            for gx, gy in territory:
                for sub_x in range(cells_per):
                    for sub_y in range(cells_per):
                        cx, cy = gx * cells_per + sub_x, gy * cells_per + sub_y
                        if 0 <= cx < CELLS_X and 0 <= cy < CELLS_Y and self.cells[cx][cy]:
                            c = self.cells[cx][cy]
                            self.cells[cx][cy] = (int(c[0]*0.85+tint[0]*0.15), int(c[1]*0.85+tint[1]*0.15), int(c[2]*0.85+tint[2]*0.15))
    ...
    def generate_villages(self):
        # =========================================================================
        # Villages should be distributed as BUFFERS between kingdoms
        # Not clustered around one kingdom - spread across the map
        # =========================================================================
        kingdoms = [l for l in self.locations if l.loc_type == LocationType.KINGDOM]
        
        for i in range(10):
            # Find valid position - prefer positions that are BETWEEN kingdoms
            best_pos = None
            best_score = -1
            ...
            for attempt in range(200):
                ...
                # Must be away from other villages and kingdoms
                if not all(math.sqrt((x-l.x)**2+(y-l.y)**2) >= 150 for l in self.locations if l.loc_type in [LocationType.KINGDOM, LocationType.VILLAGE]):
                    continue
                
                # Score based on being between kingdoms (not too close, not too far)
                if kingdoms:
                    dists_to_kingdoms = [math.sqrt((x-k.x)**2 + (y-k.y)**2) for k in kingdoms]
                    min_dist = min(dists_to_kingdoms)
                    
                    # Sweet spot: 150-400 pixels from nearest kingdom
                    if min_dist < 120:  # Too close to a kingdom
                        continue
                    
                    # Prefer positions that are between two kingdoms
                    # (second closest kingdom shouldn't be too far)
                    sorted_dists = sorted(dists_to_kingdoms)
                    if len(sorted_dists) >= 2:
                        # Good buffer position = relatively equal distance to multiple kingdoms
                        dist_ratio = sorted_dists[0] / sorted_dists[1] if sorted_dists[1] > 0 else 0
                        # Higher ratio (closer to 1.0) = more centered between kingdoms
                        score = dist_ratio * 100 + min_dist * 0.1
                    else:
                        score = min_dist
            ...
            if best_pos:
                ...
                for _ in range(100):
                    ...
                    if not self.is_in_water(x, y) and not self.is_mountainous(x, y):
                        break
            
            v = Location(x, y, LocationType.VILLAGE, self.village_names[i], self.get_kingdom_at(x, y), size=25)
