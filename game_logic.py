...
class GameLogic:
    ...
    def check_flee(self, char):
        ...
        if char.intent and char.intent.get('action') == 'flee':
            ...
            if target and target in self.state.characters and target.health > 0:
                # Bystanders stop caring once out of perception
                if reason == 'bystander':
                    can_perceive, _ = self.can_perceive_character(char, target)
                    if not can_perceive:
                        char.clear_intent()
                        return False
                    return True
        ...
        attacker = self.get_active_attacker(char)
        if attacker:
        ...
        if char.get_trait('confidence') < 7:
            for memory in char.get_memories(memory_type='attacked_by'):
                ...
                if attacker and attacker in self.state.characters and attacker.health > 0:
                    # Is this attacker nearby (within perception)?
                    can_perceive, _ = self.can_perceive_character(char, attacker)
                    if can_perceive:
                        # Resume fleeing from this attacker
                        char.set_intent('flee', attacker, reason='being_attacked', started_tick=self.state.ticks)
                        return True
        
        return False
    
    def check_watch_threat(self, char):
    ...
    def check_fight_back(self, char):
    ...
    def check_combat(self, char):
        ...
        if char.intent.get('action') != 'attack':
            return False
        target = char.intent.get('target')
        return target and target in self.state.characters and target.health > 0
    
    def check_flee_criminal(self, char):
    ...
    def check_confront_criminal(self, char):
    ...
    def check_watch_fleeing_person(self, char):
        ...
        fleeing_person = self.find_fleeing_person_nearby(char)
        return fleeing_person is not None
    
    def find_fleeing_person_nearby(self, char):
        ...
        for other in self.state.characters:
            ...
            if other.intent and other.intent.get('action') == 'flee':
                ...
                can_perceive, _ = self.can_perceive_character(char, other)
                if can_perceive:
                    return other
        return None
    
    def check_eat(self, char):
        """Should eat?"""
    ...
    def do_flee(self, char):
        ...
        if char.intent and char.intent.get('action') in ('flee', 'watch'):
            ...
            if attacker and (attacker not in self.state.characters or attacker.health <= 0):
                attacker = None
        ...
        if flee_reason == 'bystander':
            ...
            if dist < safe_distance:
                ...
                if char.zone is not None and char.zone == attacker.zone:
                    interior = self.state.interiors.get_interior(char.zone)
                    if interior:
                        flee_x, flee_y, flee_zone = self._get_interior_flee_goal(char, attacker, interior)
                        self.set_goal_to_position(char, flee_x, flee_y, flee_zone)
                        return False
                flee_pos = self._get_flee_goal(char, attacker)
                self.set_goal_same_zone(char, flee_pos[0], flee_pos[1])
                return False
            else:
                # Safe distance - switch to watching
                char.goal = None
                char['face_target'] = None  # Clear - _face_target will handle it
                self.face_target(char, attacker)
                char.set_intent('watch', attacker, reason='bystander', started_tick=self.state.ticks)
                return False
        ...
        if char.zone is None and attacker.zone is None:
            home_interior = self.state.interiors.get_interior(char.get('home')) if char.get('home') else None
            if home_interior:
                door_x, door_y = home_interior.get_exit_position()
                if self.is_safe_direction(char, attacker, door_x, door_y):
                    ...
                    self.set_goal_to_position(char, world_x, world_y, char.get('home'))
                    return False
        ...
        defender = self.find_nearby_defender(char, VISION_RANGE * 2, exclude=attacker)
        ...
        if dist_to_attacker >= VISION_RANGE:
            char.goal = None
            self.face_target(char, attacker)
            char.set_intent('watch', attacker, reason='monitoring_threat', started_tick=self.state.ticks)
            return False
    ...
    def do_watch_threat(self, char):
        ...
        if not threat or threat not in self.state.characters or threat.health <= 0:
            char.clear_intent()
            return False
        ...
        if dist_to_threat < flee_distance:
            ...
            if reason == 'bystander':
                char.set_intent('flee', threat, reason='bystander', started_tick=self.state.ticks)
                char['face_target'] = threat  # Backpedal while watching
            else:
                char.set_intent('flee', threat, reason='threat_approaching', started_tick=self.state.ticks)
            
            # Use interior-aware flee if in same interior
            if char.zone is not None and char.zone == threat.zone:
                interior = self.state.interiors.get_interior(char.zone)
                if interior:
                    flee_x, flee_y, flee_zone = self._get_interior_flee_goal(char, threat, interior)
                    self.set_goal_to_position(char, flee_x, flee_y, flee_zone)
                    self.state.log_action(f"{char.get_display_name()} fleeing - {threat.get_display_name()} too close!")
                    return False
            
            flee_pos = self._get_flee_goal(char, threat)
            self.set_goal_same_zone(char, flee_pos[0], flee_pos[1])
            self.state.log_action(f"{char.get_display_name()} fleeing - {threat.get_display_name()} too close!")
            return False
        ...
        if reason == 'monitoring_threat':
            ...
            if char.zone is None and threat.zone is None:
                ...
                if home_interior:
                    door_x, door_y = home_interior.get_exit_position()
                    if self.is_safe_direction(char, threat, door_x, door_y):
                        # Target center of home interior
                        center_x = home_interior.width / 2
                        center_y = home_interior.height / 2
                        world_x, world_y = home_interior.interior_to_world(center_x, center_y)
            ...
            defender = self.find_nearby_defender(char, VISION_RANGE * 2, exclude=threat)
            if defender:
                dx_to_defender = defender.x - char.x
                dy_to_defender = defender.y - char.y
                dist_to_defender = math.sqrt(dx_to_defender**2 + dy_to_defender**2)
                ...
                if dist_to_defender <= (SOUND_RADIUS * 2):
                    self.try_report_attack_to_defender(char, threat, defender)
    ...
    def try_report_attack_to_defender(self, char, attacker, defender):
        ...
        if attack_memories:
            ...
            defender.add_memory('crime', attacker, self.state.ticks,
            ...
            self.evaluate_crime_reaction(defender, attacker, memory['intensity'], char.get('allegiance'))
            self.state.log_action(f"{char.get_display_name()} told {defender.get_display_name()} about {attacker.get_display_name()}'s attack!")
    
    def do_fight_back(self, char):
        ...
        if not attacker:
            for memory in char.get_memories(memory_type='attacked_by'):
                ...
                if potential and potential in self.state.characters and potential.health > 0:
                    can_perceive, _ = self.can_perceive_character(char, potential)
                    if can_perceive:
                        attacker = potential
                        break
        ...
        if char.intent is None or char.intent.get('target') is not attacker:
            self.state.log_action(f"{char.get_display_name()} FIGHTING BACK against {attacker.get_display_name()}!")
            char.set_intent('attack', attacker, reason='self_defense', started_tick=self.state.ticks)

        # Delegate to unified combat logic (handles both melee and ranged)
        return self.do_combat(char)
    ...
    def do_combat(self, char):
        ...
        target = char.intent.get('target')
        if not target or target not in self.state.characters or target.health <= 0:
            char.clear_intent()
            char['combat_track_target'] = None
            char.cancel_bow_draw()
            return False
        ...
        if use_ranged:
            ...
            return self.do_ranged_combat(char, target, dist)
        else:
            # MELEE COMBAT: Chase and attack
            char.cancel_bow_draw()  # Cancel bow if we were drawing
            return self.do_melee_combat(char, target, dist)

    def do_melee_combat(self, char, target, dist):
        """Handle melee combat (original melee logic)."""
        # Attack if in melee range and off cooldown
        if dist <= MELEE_ATTACK_DISTANCE and self.can_attack(char):
    ...
    def do_ranged_combat(self, char, target, dist):
        ...
        if dist <= bow_range:
            ...
            ticks_since_last = self.state.ticks - last_attack_tick
            ...
            if ticks_since_last < bow_draw_time:
                ...
                char['combat_track_target'] = None
                char.goal = None
                self._face_toward_target(char, target)
                return True

            # Start drawing if not already
            if not char.is_drawing_bow():
                char.start_bow_draw(self.state.ticks)

            # Update draw progress
            draw_progress = char.get_bow_draw_progress(self.state.ticks)
            ...
            if draw_progress and draw_progress >= 0.9:
                ...
                # Fire the arrow!
                self.shoot_arrow(char, angle, draw_progress)

                # Release bow draw state
                char.release_bow_draw(self.state.ticks)

                # Record attack tick for cooldown (enforces draw time between shots)
                char['last_attack_tick'] = self.state.ticks
    ...
    def do_flee_criminal(self, char):
        ...
        if not flee_target or flee_target not in self.state.characters or flee_target.health <= 0:
            criminal, intensity = self.find_known_criminal_nearby(char)
            if criminal:
                self.state.log_action(f"{char.get_display_name()} fleeing from {criminal.get_display_name()}!")
                char.set_intent('flee', criminal, reason='known_criminal', started_tick=self.state.ticks)
        ...
        if char.zone is not None and flee_target.zone == char.zone:
            interior = self.state.interiors.get_interior(char.zone)
            if interior:
                flee_x, flee_y, flee_zone = self._get_interior_flee_goal(char, flee_target, interior)
                self.set_goal_to_position(char, flee_x, flee_y, flee_zone)
                return False
        ...
        if char.zone is None and flee_target.zone is None:
            ...
            if home_interior:
                door_x, door_y = home_interior.get_exit_position()
                if self.is_safe_direction(char, flee_target, door_x, door_y):
                    # Target center of home interior
                    center_x = home_interior.width / 2
                    center_y = home_interior.height / 2
                    world_x, world_y = home_interior.interior_to_world(center_x, center_y)
        ...
        # Look for any defender in range
        defender = self.find_nearby_defender(char, VISION_RANGE * 2, exclude=flee_target)
        ...
        if defender:
            ...
            if can_report_via_sound:
                ...
                self.try_report_crimes_to_defender(char, flee_target, defender)
        ...
        if dist_to_criminal >= VISION_RANGE:
            ...
            char.goal = None
            self.face_target(char, flee_target)
            # Clear flee intent since we're just watching now
            char.set_intent('watch', flee_target, reason='monitoring_threat', started_tick=self.state.ticks)
            return False
        
        if dist_to_criminal < VISION_RANGE / 2:
            # Too close - flee!
            flee_pos = self._get_flee_goal(char, flee_target)
            self.set_goal_same_zone(char, flee_pos[0], flee_pos[1])
            return False
    ...
    def is_safe_direction(self, char, threat, target_x, target_y):
    ...
    def try_report_crimes_to_defender(self, char, criminal, defender):
        ...
        if crimes:
            ...
            memory = crimes[-1]
            self.report_crime_to(char, defender, memory)
            self.state.log_action(f"{char.get_display_name()} told {defender.get_display_name()} about {criminal.get_display_name()}'s crime!")
    
    def do_confront_criminal(self, char):
        ...
        if char.intent is None or char.intent.get('target') is not criminal:
            self.state.log_action(f"{char.get_display_name()} confronting {criminal.get_display_name()}!")
            char.set_intent('attack', criminal, reason='confronting_criminal', started_tick=self.state.ticks)
    ...
    def do_watch_fleeing_person(self, char):
        ...
        if not fleeing_person or fleeing_person not in self.state.characters:
            fleeing_person = self.find_fleeing_person_nearby(char)
        ...
        can_perceive, _ = self.can_perceive_character(char, fleeing_person)
        if not can_perceive:
            char.clear_intent()
            return False
        ...
        if char.intent is None or char.intent.get('target') is not fleeing_person:
            char.set_intent('watch', fleeing_person, reason='monitoring_distress', started_tick=self.state.ticks)
            self.state.log_action(f"{char.get_display_name()} noticed {fleeing_person.get_display_name()} fleeing!")
        
        # Stay in place and face the fleeing person
        char.goal = None
        self.face_target(char, fleeing_person)
        
        return False
    
    def do_eat(self, char):
        """Eat bread."""
        result = char.eat()
        if result['success']:
            name = char.get_display_name()
            if result.get('recovered_from_starvation'):
                self.state.log_action(f"{name} ate bread and recovered from starvation! Hunger: {char.hunger:.0f}")
            else:
                self.state.log_action(f"{name} ate bread, hunger now {char.hunger:.0f}")
            return True
        return False
    
    def do_cook(self, char):
        ...
        if bed:
            ...
            if char.zone == bed.zone and dist < 0.15:
                if not char.get('is_sleeping'):
                    ...
                    self.state.log_action(f"{char.get_display_name()} went to sleep")
                char.goal = None
                char.goal_zone = None
                return True
            else:
    ...
    def do_wander(self, char):
        """Wander aimlessly within home area or interior."""
        home = char.get('home')
        if home:
            # _get_wander_goal handles both exterior areas and interior wandering
            wander_goal = self._get_wander_goal(char, home)
            if wander_goal:
                self.set_goal_same_zone(char, wander_goal[0], wander_goal[1])
            else:
                char.goal = None
        else:
            # Homeless wandering (exterior only)
            wander_goal = self._get_homeless_idle_goal(char)
            if wander_goal:
                self.set_goal_same_zone(char, wander_goal[0], wander_goal[1])
            else:
                char.goal = None
    ...
    def do_forage(self, char):
        ...
        if char.hunger <= HUNGER_CRITICAL:
            ...
            ready_cell = self.find_nearby_ready_farm_cell(char)
            if ready_cell:
    ...
    def check_fight_back_soldier(self, char):
        ...
        return self.get_active_attacker(char) is not None
    
    def do_confront_criminal_soldier(self, char, criminal):
        """Soldiers confront criminals aggressively."""
        # Set attack intent if not already targeting this criminal
        if char.intent is None or char.intent.get('target') is not criminal:
            self.state.log_action(f"{char.get_display_name()} confronting {criminal.get_display_name()}!")
            char.set_intent('attack', criminal, reason='law_enforcement', started_tick=self.state.ticks)
    ...
    def do_patrol(self, char):
        """Patrol between waypoints."""
        waypoints = self.state.get_patrol_waypoints()
        if not waypoints:
            return self.do_wander(char)
    ...
    def get_player_nearby_corpse(self, player):
        """Get corpse player is facing and close enough to interact with.
        
        Args:
            player: Player character
            
        Returns:
            Corpse object if one is available, None otherwise
        """
        from constants import INTERACT_DISTANCE
    ...
    def get_player_nearby_barrel(self, player):
        """Get barrel player is facing and close enough to interact with.
        
        Args:
            player: Player character
            
        Returns:
            Barrel object if one is available, None otherwise
        """
        for barrel in self.state.interactables.barrels.values():
            if not barrel.is_adjacent(player):
                continue
    ...
    def get_player_nearby_bed(self, player):
        """Get bed player is facing and close enough to interact with.
        
        Args:
            player: Player character
            
        Returns:
            Bed object if one is available, None otherwise
        """
    ...
    def get_player_nearby_tree(self, player):
        """Get tree player is facing and close enough to interact with.
        
        Args:
            player: Player character
            
        Returns:
            Tree object if one is available, None otherwise
        """
    ...
    def is_char_in_attack_cone(self, attacker, target, attack_angle):
        """Check if a character is in an attacker's attack cone.

        Uses 360° angle-based cone detection.
        Cone interpolates from BASE_ANGLE at attacker to full ANGLE at weapon reach.

        Args:
            attacker: Attacking character
            target: Target character to check
            attack_angle: Attack angle in radians

        Returns:
            True if target is in the attack cone
        """
        from constants import ATTACK_CONE_BASE_ANGLE, ATTACK_CONE_ANGLE
        import math
        ...
        # Get weapon reach
        weapon_stats = attacker.get_weapon_stats()
        weapon_reach = weapon_stats.get('range', 1.0)

        # Calculate relative position using prevailing coords
        rel_x = target.prevailing_x - attacker.prevailing_x
        rel_y = target.prevailing_y - attacker.prevailing_y
        ...
        t = distance / weapon_reach  # 0 at attacker, 1 at max range
    ...
    def get_chars_in_attack_cone(self, attacker, attack_angle):
        """Get all characters currently in an attacker's attack cone.
        ...
        Returns:
            Set of character objects in the attack cone
        """
        chars_in_cone = set()
        for char in self.state.characters:
            if self.is_char_in_attack_cone(attacker, char, attack_angle):
                chars_in_cone.add(char)
    ...
    def get_active_attacker(self, char, max_ticks_ago=50, max_distance=8.0):
        """Find someone actively attacking a character right now.

        Checks for recent attacked_by memories where attacker is still nearby.

        Args:
            char: Character to check
            max_ticks_ago: How recent the attack must be (default 50 ticks)
            max_distance: How close attacker must still be (default 8.0 cells)
        ...
        for m in char.get_memories(memory_type='attacked_by'):
            # Recent?
            if self.state.ticks - m['tick'] > max_ticks_ago:
                continue

            attacker = m['subject']

            # Still alive?
            if attacker not in self.state.characters or attacker.health <= 0:
                continue

            # Still nearby?
            dist = math.sqrt((char.x - attacker.x)**2 + (char.y - attacker.y)**2)
            if dist < max_distance:
                return attacker
